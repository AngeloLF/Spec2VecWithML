...
def inspect_training(params):
	...
	if "models_output" not in os.listdir("./results"):

		rl = f"Training {model_name} with {load_name}{train_name} : {c.r}{c.tu}{c.ti}Not exist ...{c.d}"
		ra = -1

	if model_name in os.listdir(f"./results/models_output") and "epoch" in os.listdir(f"./results/models_output/{model_name}") and f"{load_name}{train_name}" in os.listdir(f"./results/models_output/{model_name}/epoch/"):

		nb_make = len(os.listdir(f"./results/models_output/{model_name}/epoch/{load_name}{train_name}"))
		lmax = len(str(epoch))
